#!/usr/bin/env nu
rm -rf ./nix/hosts/server/server/*
mkdir ./nix/hosts/server/server
cp -r ./tools/server/* ./nix/hosts/server/server/
...
try {
  ...
  scp -r -P $port ./nix env-admin@localhost:/home/env-admin/nixos
